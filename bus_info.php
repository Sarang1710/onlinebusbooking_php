                        ...
                        <div class="container-fluid">
                            ...
                            if (isset($_POST['book'])) {
                                ...
                                if (array_search($source, $bus_stations) < array_search($destination, $bus_stations)) {
                                    ...
                                    $row = mysqli_fetch_assoc($get_cost);
                                    $cost = $row['cost'];
                                    ...
                                    for ($i = 0; $i < $count; $i++) {
                                        ...
                                        $query_payment = "SELECT order_id FROM orders WHERE bus_id = $selected_bus AND user_id = $user_id AND user_name = '$curr_name'AND user_age = '$curr_age'AND source = '$source' AND destination = '$destination'  AND cost = $cost";
                                        $get_oid = mysqli_query($connection, $query_payment);
                                        $row = mysqli_fetch_assoc($get_oid);
                                        $o_id = $row['order_id'];
                                        $query_pay_entry = "INSERT INTO payment(pay_id,pay_price) VALUES($o_id,$cost)";
                                        $payment_done = mysqli_query($connection, $query_pay_entry);
                                    }
                            ?>
                                    <form action="" method="post">
                                        <div class="container">
                                            <div class="card-title d-flex px-4">
                                                <p class="item text-muted"><?php echo $source . " to " . $destination; ?></p>
                                                <p>Rs. <?php echo $tcost; ?></p>
                                            </div>

                                            <input type="hidden" name="result" value="<?php echo $payment_done ?>">
                                            <div class="card-body">
                                                <p class="text-muted">Your payment details</p>
                                                <div class="form-group">
                                                    <label class="control-label col-sm-2" for="email"></label>
                                                    <div class="col-sm-10">
                                                        <input type="text" class="form-control" id="email" placeholder="Card Number" name="card_no">
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label col-sm-2" for="email"></label>
                                                    <div class="col-sm-10">
                                                        <input type="text" class="form-control" id="email" placeholder="Card Holder Name" name="card_holder">
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label col-sm-2" for="email"></label>
                                                    <div class="col-sm-5">
                                                        <input type="text" class="form-control" id="email" placeholder="MM/YY" name="m_y">
                                                    </div>
                                                    <label class="control-label col-sm-2" for="email"></label>
                                                    <div class="col-sm-5">
                                                        <input type="text" class="form-control" id="email" placeholder="CVV" name="cvv">
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="footer text-center ">
                                                <button class="btn btn-primary" name="pay" style="margin-left: 40%; margin-top: 15px;">BOOK</button>
                                            </div>
                                        </div>
                                    </form>
                            <?php
